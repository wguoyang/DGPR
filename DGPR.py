...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='param/bpr_basedir/ml1m/DGPR_gcn_gender_0.10.pth')
    parser.add_argument('--pretrain_path', type=str, default='param/bpr_basedir/ml1m/new_gcn_base_0.02_ml1m.pth')
    parser.add_argument('--pretrain_bprmf_loadpath',type=str,default='param/bpr_base.pth')
    parser.add_argument('--lreg', type=float, default=0.01)
    ...
    if args.replace == True:
        if att=='gender':
            new_train_u2i = sevsitive_gender_side_dg(pretrainuemb,pretrainiemb,args.replace_ratio,u_sens,att,'ml1m')
        else:
            new_train_u2i = sevsitive_ageocc_side_dg(pretrainuemb, pretrainiemb, args.replace_ratio, u_sens, att, 'ml1m')
        print('敏感信息侧数据集已生成')
    ...
    if att=='gender':
        ...
        classifier = classifierage_occ(outdim=7).cuda()
    else:
        classifier = classifierage_occ(outdim=21).cuda()
